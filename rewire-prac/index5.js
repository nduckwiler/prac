    ...
    'FindVideoByGenreIntent' : function() {
        ...
        if (this.event.request.dialogState !== 'COMPLETED'){
          ...
          if (!this.event.request.intent.slots.genre.value) {
            ...
            // the genre slot has been collected and confirmed
            if (intentConfirmationStatus == 'DENIED'){
              // check if the intent confirmation was denied
              this.response.speak('Ok, canceling...What genre would you like?').listen('What genre would you like?');
              this.emit(':responseReady');
            }
            else{
              // if intent confirmation status is CONFIRMED or NONE, delegate control back to Alexa
              this.emit(':delegate');
            }
          }
        }
        else {
          //DIALOG STATE: Completed. All required slots have been filled
          ...
          const genreRequestedByUser = this.event.request.intent.slots.genre.value;
          const decade = randomize(["70s","80s","90s"]); //randomly choosing a decade if not provided by the user
