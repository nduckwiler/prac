...
describe('FindVideoByGenreIntent-L3E2', () => {
  ...
  it('calls this.emit with :elicitSlot when dialog in progress and genre not collected', () => {
    // Set up your test with the desired conditions
    handlers.event.request.dialogState = 'IN_PROGRESS';
    delete handlers.event.request.intent.slots.genre.value;
    handlers.event.request.intent.slots.videoType.value = 'movies';
  ...
  it('calls this.emit with :delegate when dialog is not completed but genre collected', () => {
    // Set up your test with the desired conditions
    handlers.event.request.dialogState = 'IN_PROGRESS';
    handlers.event.request.intent.slots.genre.value = 'comedy';
    handlers.event.request.intent.slots.genre.resolutions.resolutionsPerAuthority[0].values[0].value.name = 'comedy';
    delete handlers.event.request.intent.slots.videoType.value;
    delete handlers.event.request.intent.slots.videoType.resolutions.resolutionsPerAuthority[0].values[0].value.name;
